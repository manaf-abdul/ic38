...
import ConceptsContentModal from "../Components/Modals/ConceptsContentModal";
...
const ConceptsContent = () => {
  ...
  const fetchData = async () => {
    const { data } = await axios.get(`${BASEURL}/api/concepts/${params.id}`);
  ...
  const editHandler = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${BASEURL}/api/concepts/data/edit`,
        name
      );
      if (data.errorcode === 0) {
  ...
  return (
    <>
      ...
      <Container>
        {terminologyData && terminologyData.length > 0 ? (
