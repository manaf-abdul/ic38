...
const PractiseTestModal = (props) => {
    ...
    const addHandler = async (selected) => {
        try {
            const { data } = await axios.post(`${BASEURL}/api/practisetest/question/add`, { q: question, a: answer, option: who, category: category, language: language, id: params?.id })
            if (data.errorcode === 0) {
    ...
    const editSingleHandler = async (selected) => {
        try {
            const { data } = await axios.post(`${BASEURL}/api/practisetest/question/edit`, { q: question, a: answer, o1, o2, o3, o4, category: category, language: language, id: props?.edit?._id })
            if (data.errorcode === 0) {
                toast.success(`🦄 ${data.msg}!`, successToast);
                props?.setRender()
                setAnswer('')
    ...
    const editHandler = async (selected) => {
        try {
            ...
            try {
                ...
                const { data } = await axios.post(`${BASEURL}/api/practisetest/question-file`, formData, config)
                if (data.errorcode === 0) {
                    toast.success(`🦄 ${data.msg}!`, successToast);
                    props?.setRender()
                    setAnswer('')
...
export default PractiseTestModal