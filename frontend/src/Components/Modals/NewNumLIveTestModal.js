...
const NewNumMockTestModal = (props) => {
    ...
    const addHandler = async (selected) => {
        try {
            const { data } = await axios.post(`${BASEURL}/api/livetest/question/add`, { q: question, a: answer, option: who, category: category, language: language, id: params?.id })
            if (data.errorcode === 0) {
    ...
    const editSingleHandler = async (selected) => {
        try {
            const { data } = await axios.post(`${BASEURL}/api/livetest/question/edit`, { q: question, a: answer, o1, o2, o3, o4, category: category, language: language, id: props?.edit?._id })
            if (data.errorcode === 0) {
                toast.success(`🦄 ${data.msg}!`, successToast);
                props?.setRender()
                setAnswer('')
            ...
            >
                ...
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-title-vcenter">
                        {props.bulk ? "Add Live tests" : "Livetest"}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {props.edit ?
